...
class ExplicitRungeKuttaMethod:
    ...
    def __init__(
        ...
        start_value: Union[np.ndarray, float],
        start_time: float = 0,
    ):
        self._right_hand_side_function = right_hand_side_function
        self._build_solution(start_value)
        self._time = start_time

    def _build_solution(self, start_value: Union[float, np.ndarray]):
    ...
    def execute_step(self, delta_t: float):
        ...
        for index in range(stage_number):
            stage_value = self.solution + delta_t * np.sum(
                np.array(
                    [
            ...
            stages.append(self._right_hand_side_function(stage_value))

        self._solution += delta_t * np.sum(
            np.array([weight * stage for weight, stage in zip(self._weights, stages)]),
            axis=0,
        )
        self._time += delta_t

    def execute_steps(self, target_time: float, time_steps_number: int):
        """TIME_STEPS_NUMBER Runge-Kutta steps until the TARGET_TIME is reached."""
        time_grid = np.linspace(self._time, target_time, time_steps_number + 1)

        for i in range(len(time_grid) - 1):
            delta_t = time_grid[i + 1] - time_grid[i]
            self.execute_step(delta_t)
...
class StrongStabilityPreservingRungeKutta4(ExplicitRungeKuttaMethod):
    ...
    def _calculate_first_stage(self, delta_t: float):
        self._stages[0] = self.solution + (
            ...
            * delta_t
            * self._right_hand_side_function(self.solution)
        )
    ...
    def _calculate_second_stage(self, delta_t: float):
        self._stages[1] = (
            ...
            * delta_t
            * self._right_hand_side_function(self._stages[0])
        )
    ...
    def _calculate_third_stage(self, delta_t: float):
        self._stages[2] = (
            ...
            * delta_t
            * self._right_hand_side_function(self._stages[1])
        )
    ...
    def _calculate_fourth_stage(self, delta_t: float):
        self._stages[3] = (
            ...
            * delta_t
            * self._right_hand_side_function(self._stages[2])
        )
    ...
    def _calculate_new_solution(self, delta_t: float) -> np.ndarray:
        return (
            ...
            * delta_t
            * self._right_hand_side_function(self._stages[3])
        )
...
class RungeKutta8(ExplicitRungeKuttaMethod):
    ...
    def __init__(
        ...
        ExplicitRungeKuttaMethod.__init__(
            self, right_hand_side_function, start_value, start_time
        )
