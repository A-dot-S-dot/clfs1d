...
class TestUniformMesh(TestCase):
    ...
    def test_find_simplex(self):
        for point, simplices in zip(self.points, self.expected_simplices):
            self.assertListEqual(self.mesh.find_simplex(point), simplices)

    def test_find_simplex_index(self):
        for point, indices in zip(self.points, self.expected_indices):
            self.assertListEqual(self.mesh.find_simplex_index(point), indices)
